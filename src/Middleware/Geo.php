...
class Geo extends Middleware
{
    ...
    protected function ipapipro($location)
    {
        ...
        if(App::environment('local')){
            $response = $this->getResponse('https://pro.ip-api.com/json/' . '209.139.228.193' . '?fields=continent,country,regionName,city,proxy,hosting&key='. config('firewall.middleware.geo.api_key'));
        }
        else{
